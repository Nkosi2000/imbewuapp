...
@if(session()->has('message') || $slot->isNotEmpty())
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
         class="mb-4 border-l-4 p-4 rounded-md shadow-md flex justify-between items-center {{ $types[$type] }}">
        <div class="flex items-center">
            ...
            @endif
            <span>{{ session('message') ?? $slot }}</span>
        </div>
        <button @click="show = false" class="opacity-75 hover:opacity-100 text-gray-400 hover:text-white">
